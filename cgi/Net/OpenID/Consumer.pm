...
sub verified_identity {
    ...
    foreach my $endpoint (@$possible_endpoints) {
		...
		# Changed by Fletcher Penney to avoid problem with myopenid
		my $trim_endpoint = $endpoint_uri;
		my $trim_server = $server;
		$trim_server =~ s/^https?//;
		$trim_endpoint =~ s/^https?//;
		
        if ($trim_endpoint ne $trim_server) {
